...
export default function RatingsPage() {
  ...
  return <main className="ratingsPage">
    <header className="ratingsHeader"><div><p className="eyebrow">ВНУТРЕННИЙ ИНСТРУМЕНТ · БАЗА ИГРОКОВ</p><h1>RATINGS<br/><span>REVIEW</span></h1></div><div className="headerActions"><div className="databaseCount"><b>{versions.length}</b><span>СЕЗОННЫХ ВЕРСИЙ</span></div><button className={editor ? 'editorButton active' : 'editorButton'} onClick={() => setEditor((value) => !value)}>{editor ? 'ВЫЙТИ ИЗ РЕЖИМА' : 'РЕЖИМ РЕДАКТОРА'}</button></div></header>
